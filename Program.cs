...
class Program
{
    ...
    static void tampilkanIntro(){
        Console.Clear();
        Console.WriteLine("Ini adalah game mobil");
        Console.WriteLine();
        Console.WriteLine("Arahkan mobil agar tetap berada di jalan raya");
        Console.WriteLine();
        Console.WriteLine("Tekan A,W,dan D untuk mengarahkan mobil");
        Console.WriteLine();
        Console.Write("Tekan (Enter) untuk memulai....");
        ...
        for(int i=height - 1;i>= 0;i--){
            for(int j = 0;j < width;j++){
                if(i == 1 && j==posisiMobil){
                    stringBuilder.Append(!mulaiMain ? 'X':
                                        kecepatanMobil<0?'<':
                                        kecepatanMobil>0?'>':
                                        '^');
                }else{
